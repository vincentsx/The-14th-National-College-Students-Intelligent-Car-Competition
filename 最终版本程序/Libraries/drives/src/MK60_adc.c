...
ADC_Type * const ADCX[] = ADC_BASES; //定义两个指针数组保存 ADCN 的地址
...
//          3次ADC转换，选取中间值返回。                   
//-------------------------------------------------------------------------*
uint16 ADC_Mid(ADCn adc_n,ADCn_Ch adc_ch,ADC_nbit bit) //中值滤波
{
    uint16 i,j,k,tmp;
    //1.取3次A/D转换结果
...
//          100次ADC转换，计算出平均值返回。                   
//-------------------------------------------------------------------------*
uint16 adc_ave(ADCn adc_n,ADCn_Ch adc_ch,ADC_nbit bit,uint16 N) //均值滤波
{
    uint32 tmp = 0;
    uint8  i;
